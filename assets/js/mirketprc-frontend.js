/**
 * Mirket Popup Redirect Countdown — Frontend JS
 *
...
(function () {
    ...
    var settings = window.mirketprcSettings || {};
    ...
    var redirectingText = settings.redirectingText || 'Redirecting in';

    /* ---- Guard: already dismissed ---- */

    if (cookieDays > 0 && getCookie('mirketprc_dismissed') === '1') {
        return;
    }
    ...
    onReady(function () {
        var overlay       = document.getElementById('mirketprc-overlay');
        var backdrop      = overlay ? overlay.querySelector('.mirketprc-overlay__backdrop') : null;
        var closeBtn      = overlay ? overlay.querySelector('.mirketprc-overlay__close') : null;
        var countdownText = document.getElementById('mirketprc-countdown-text');
        var progressFill  = document.getElementById('mirketprc-progress-fill');
        ...
        function show() {
            overlay.classList.add('mirketprc-overlay--visible');
            updateCountdownText();
            startProgressBar();
        ...
        function updateCountdownText() {
            if (countdownText) {
                countdownText.textContent = redirectingText + ' ' + remaining + ' second' + (remaining !== 1 ? 's' : '') + '…';
            }
        }
        ...
        function dismiss() {
            clearInterval(timer);
            overlay.classList.remove('mirketprc-overlay--visible');
            ...
            /* Set cookie */
            if (cookieDays > 0) {
                setCookie('mirketprc_dismissed', '1', cookieDays);
            }
        }
        ...
        var content = overlay.querySelector('.mirketprc-overlay__content');
        if (content) {
        ...
        document.addEventListener('keydown', function (e) {
            if (e.key === 'Escape' && overlay.classList.contains('mirketprc-overlay--visible')) {
                dismiss();
            }
        });
